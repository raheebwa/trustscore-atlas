...
"""Conformance checks for the URA VAT withholding agents register."""
...
def run(spec, fetcher, tmp_path_factory):
    return _run(spec, fetcher, tmp_path_factory.mktemp("ura-vat-run"))
...
def test_4_identifier_values_match_pack_patterns(run):
    ...
    assert {item["scheme"] for item in identifiers} == {"ug:tin"}
...
def test_report_code_and_request_parameters_match_the_reference(spec):
    assert spec.module.REPORT_CODE == 1004
    assert spec.module.EXTRA_PARAMS == {}
    assert spec.module.first_page_data("fixture-token")["reportCode"] == "1004"
