...
ADAPTER = PACKS / "ug" / "sources" / "ura_wht_exemptions"
...
def run(spec, fetcher, tmp_path_factory):
    return _run(spec, fetcher, tmp_path_factory.mktemp("ura-wht-run"))
...
def test_report_code_and_request_parameters_match_the_reference(spec):
    assert spec.module.REPORT_CODE == 7
    assert spec.module.EXTRA_PARAMS == {}
    assert spec.module.first_page_data("fixture-token")["reportCode"] == "7"
