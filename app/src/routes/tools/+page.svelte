<script lang="ts">
	...
	import { pageTools } from '$lib/webmcp/registered.svelte';
	...
	// Whether the browser offers an agent surface, which changes who else can call these tools and
	// nothing about whether a visitor can.
	let agentSurface = $state<boolean | null>(null);
	let running = $state<string | null>(null);
	let copied = $state<string | null>(null);
	let results = $state<Record<string, ExecutionView>>({});

	// The page runs what it registered: the same descriptions, the same schemas and the same code
	// an agent would reach, rather than a second implementation that could drift from it.
	const actions = $derived(
		pageTools.current
			.map((tool) => ({
				...
				schema: normaliseInputSchema(tool.inputSchema),
	...
	async function runAction(event: SubmitEvent, action: (typeof actions)[number]) {
		...
		try {
			const result = await action.execute(argumentsObject as never);
			const returned = result.content?.[0]?.text ?? JSON.stringify(result);
	...
	onMount(() => {
		...
		agentSurface = Boolean(candidate && typeof candidate.getTools === 'function');
	});
</script>

<svelte:head><title>TrustScore Atlas: Tools</title></svelte:head>
...
<div class="flex flex-col gap-6">
	...
	{#if agentSurface === false}
		<Callout tone="info" title="An agent cannot see these tools in this browser">
			Everything below still runs here, in this page. For an agent in the browser to call the same
			tools, it needs {CHROME_REQUIREMENT}
		</Callout>
	{/if}
	...
	{#if actions.length === 0}
		...
		{:else}
			<EmptyState
				...
				body="The page offers its tools when it loads. Reload, or open a business record, where the record-specific actions are offered."
			...
			>
				{#each actions as action (action.registered.name)}
					...
					>
						...
						<span class="line-clamp-2 text-xs text-ink-muted">{action.registered.description}</span>
					</a>
				{/each}
			</nav>
