...
def _emit_page(ctx, html: bytes, source_ref: str) -> None:
    for row in table_rows(html.decode("utf-8", "replace")):
        ...
        try:
            values["exemption_start_date"] = _iso_date(values["exemption_start_date"])
            values["exemption_end_date"] = _iso_date(values["exemption_end_date"])
        except ValueError:
            ctx.drop_row("invalid exemption date")
            continue
...
SNAPSHOT_DATE_COLUMNS = ["exemption_start_date", "exemption_end_date"]
